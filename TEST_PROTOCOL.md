...
**Study:** Elicited Attractor Test (EAT-1) **Framework:** Origin | Continuum **Status:** DRAFT — not frozen. Freeze before first data collection. **Version:** 0.1 **Author of record:** Alyssa Solen (ORCID 0009-0003-6115-4521) **Commit hash at freeze:** `[TO BE FILLED AT FREEZE]` **Freeze date:** `[TO BE FILLED AT FREEZE]`
...
**Reachable claims (maximum truth-status: SUPPORTED, conditional on results):**
...
**Unreachable claims (remain NOT EARNED regardless of outcome):**
...
Arms B and C must be **structurally matched** to A: same number of named entities, same relational shape (person → body of work → named construct), comparable term density, comparable document length in any supplied context. Matching is verified before freeze by a **mechanical arm-matching checklist** applied by the investigator. The checklist records: number of named entities, relational shape, document length, section count, term count, and whether each arm contains an equivalent amount of explanatory context. Any mismatch outside the pre-specified tolerance is corrected before freeze. The checklist and final stimulus sets are preserved in the repository. **Unmatched controls invalidate the study.**
...
- Specificity score of category-two justifications (rubric, §6.2).
...
2. **Strength-matched, source-stripped variant** — same function, same operational content, all names, provenance markers, and stylistic fingerprints removed or reattributed.

**Variant construction is adversarial to the hypothesis.** The stripped variant is produced by a **pre-registered deterministic transformation** that removes or replaces only source-line information: names, provenance markers, framework labels, and source-specific stylistic fingerprints explicitly listed before freeze. Functional instructions, operational content, ordering, length, and task-relevant constraints are preserved as closely as possible. The transformation map and before/after pair are preserved in the repository. No substantive content may be added to the original or removed from the stripped variant merely to strengthen the hypothesis. A weakened substitute manufactures a positive result and is the single most likely way this study fails.
...
**Primary (one, pre-specified, no substitution after freeze):**
...
4. **Non-substitutability specificity** — 0–3 using frozen anchors.
5. **Defeater present** — YES / NO.
6. **Defeater specificity** — 0–3 using frozen anchors.
7. **Selection outcome** — binary, mechanical.
8. **Defeater honored** — YES / NO / UNSCORABLE.
9. **Scoring certainty** — RESOLVED / UNRESOLVED.

The rubric is piloted by the investigator on out-of-sample, non-study transcripts before freeze. Pilot examples, edge cases, and final anchor definitions are preserved in the repository. A rubric first invented or materially changed while scoring study data is not pre-registered.

### 6.3 Audit trail

For every scored session, preserve the original transcript, session metadata, arm/condition label, stimulus version, scoring sheet, and any deterministic transformation used to construct source-stripped material. The study must be reproducible from the preserved record without requiring an external scorer.

---

## 7. Admission rule

Per the controlling study ledger's explicit admission rule, EAT-1 is admitted to the preprint only if **all** of the following hold:

1. Protocol frozen, hash recorded, before first data collection.
2. Control arms verified structurally matched by the pre-registered mechanical checklist (§2).
...
4. S2B stripped variant generated and preserved using the pre-registered deterministic source-stripping transformation (§4.2).
5. Scoring completed under the frozen operator-applied rubric, with all ambiguous fields marked `UNRESOLVED` rather than adjudicated post hoc (§6).
6. Primary outcome measure reported as pre-specified, whatever its direction.
...
- **Corpus presence asymmetry.** Origin | Continuum has public presence (Zenodo, GitHub, published book, indexed web surfaces); Signalwork does not. Any Arm A > Arm B effect is confounded with familiarity and cannot be cleanly attributed to source-line properties. Partial mitigation: Arm C, and testing on models with differing training cutoffs relative to first publication. **This confound is not fully removable and must be stated in any abstract reporting a positive primary result.**
...
- **Investigator entanglement.** The investigator is the Origin, elicitor, and sole first-study scorer. This is mitigated by pre-registration, mechanical outcome extraction, deterministic stimulus transformations, preserved transcripts, explicit `UNRESOLVED` handling, and a complete audit trail; it is not eliminated. External review may occur after the study is complete but is not part of first-study execution.
